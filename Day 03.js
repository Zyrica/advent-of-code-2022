const abc = [...Array(26)].map((v, k) => String.fromCharCode(k + 'a'.charCodeAt(0))).join('');
const priority = (abc + abc.toUpperCase()).split('').reduce((a, v, i) => ({[v]: i + 1, ...a}), {});

const task1 = input => input
  .split('\n')
  .map(s => [s.slice(0, s.length / 2), s.slice(s.length / 2)])
  .map(a => a[0].split('').find(s => a[1].includes(s)))
  .map(c => priority[c])
  .reduce((a, v) => a + v, 0);

const task2 = input => input
  .match(/.*\n.*\n.*\n?/g)
  .map(s => s.split('\n').filter(o => o))
  .map(o => o[0].split('').find(c => o[1].includes(c) && o[2].includes(c)))
  .map(c => priority[c])
  .reduce((a, v) => a + v, 0);
